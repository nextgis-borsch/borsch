...
function(find_anyproject name)
    ...
    # Normalize WITH_ options so both WITH_${name} (e.g. WITH_Iconv) and
    # WITH_${UPPER_NAME} (e.g. WITH_ICONV) are accepted.
    if(NOT DEFINED WITH_${name} AND DEFINED WITH_${UPPER_NAME})
        set(WITH_${name} ${WITH_${UPPER_NAME}})
    endif()
    if(NOT DEFINED WITH_${name}_EXTERNAL AND DEFINED WITH_${UPPER_NAME}_EXTERNAL)
        set(WITH_${name}_EXTERNAL ${WITH_${UPPER_NAME}_EXTERNAL})
    endif()
    # Also accept WITH_<X> for packages named Lib<X>, e.g. LibKML <- KML.
    if(name MATCHES "^Lib(.+)$")
        set(_NAME_NO_LIB "${CMAKE_MATCH_1}")
        string(TOUPPER "${_NAME_NO_LIB}" _NAME_NO_LIB_UPPER)
        foreach(_alias IN ITEMS "${_NAME_NO_LIB}" "${_NAME_NO_LIB_UPPER}")
            if(DEFINED WITH_${_alias})
                set(WITH_${name} ${WITH_${_alias}})
            endif()
            if(DEFINED WITH_${_alias}_EXTERNAL)
                set(WITH_${name}_EXTERNAL ${WITH_${_alias}_EXTERNAL})
            endif()
        endforeach()
    endif()
    ...
    if(WITH_${name})
        ...
        if(WITH_${name}_EXTERNAL)
            include(${CMAKE_CURRENT_FUNCTION_LIST_DIR}/FindExtProject.cmake)
            find_extproject(${name} ${ARGN})
            # Normalize results from external package lookups. Some packages
            # provide variables with a shorter alias (e.g. KML_* for LibKML).
            if(DEFINED ${name}_FOUND)
                if((NOT ${IS_FOUND}) AND ${name}_FOUND)
                    set(${IS_FOUND} TRUE)
                endif()
            endif()
            if(name MATCHES "^Lib(.+)$")
                string(TOUPPER "${CMAKE_MATCH_1}" _NAME_NO_LIB_UPPER_LOCAL)
                if(DEFINED ${_NAME_NO_LIB_UPPER_LOCAL}_FOUND)
                    if((NOT ${IS_FOUND}) AND ${${_NAME_NO_LIB_UPPER_LOCAL}_FOUND})
                        set(${IS_FOUND} TRUE)
                    endif()
                endif()
                foreach(_var IN ITEMS INCLUDE_DIR INCLUDE_DIRS LIBRARY LIBRARIES VERSION VERSION_STR VERSION_STRING)
                    if((NOT DEFINED ${UPPER_NAME}_${_var} OR "${${UPPER_NAME}_${_var}}" STREQUAL "") AND
                       DEFINED ${_NAME_NO_LIB_UPPER_LOCAL}_${_var} AND NOT "${${_NAME_NO_LIB_UPPER_LOCAL}_${_var}}" STREQUAL "")
                        set(${UPPER_NAME}_${_var} ${${_NAME_NO_LIB_UPPER_LOCAL}_${_var}})
                    endif()
                endforeach()
            endif()
        else()
