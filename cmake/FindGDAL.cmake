...
if(NOT GDAL_FOUND OR NOT TARGET GDAL::GDAL)
  ...
  IF (GDAL_FOUND)
    ...
    # Ensure GDAL_LIBRARY is an absolute path to avoid creating a fake 'gdal' target dependency
    if (NOT IS_ABSOLUTE "${GDAL_LIBRARY}")
     # Try to resolve to a full path; fallback to common name 'gdal' if variable contains the bare name
     find_library(_GDAL_LIB_PATH NAMES ${GDAL_LIBRARY} gdal)
     if (_GDAL_LIB_PATH)
      set(GDAL_LIBRARY "${_GDAL_LIB_PATH}" CACHE FILEPATH "Path to GDAL library" FORCE)
     else()
      message(FATAL_ERROR "FindGDAL.cmake: GDAL_LIBRARY is not an absolute path ('${GDAL_LIBRARY}') and could not be resolved to a library file.")
     endif()
    endif()
